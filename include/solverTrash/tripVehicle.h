...
class Trip: public Vehicle1 {
  public:
      ...
      void getNodesOnPath(const Trip &o_trip, POS o_ins_pos, TwBucket &nodesOnPath) const;
      void getNodesOnPath(const Trip &o_trip, TwBucket &nodesOnPath) const;
      void getNodesNotOnPath(const Trip &o_trip, TwBucket &nodesNotOnPath) const;
      double  delta_del(POS del_pos) const;
      double  delta_ins(UINT n_nid, POS del_pos) const;
      int&   trip_id() {return m_trip_id;} 
      void removeRestricted(TwBucket &nodesOnPath, POS special) const;
...
class Vehicle: public Vehicle1 {
 public:
     ...
     Vehicle():Vehicle1(){}
     Vehicle(const std::string &line, const TwBucket &otherlocs )
     ...
     int  exchangesWorse(Trip &trip, Trip &o_trip, UINT lim_iter);
