...
class Twpath : public TwBucket {
 private:
     ...
     typedef typename std::deque<Trashnode>::const_reverse_iterator
 ...
 public:
     ...
     // ------------------------------------------------------------------
     using TwBucket::swap;
     using TwBucket::insert;
     using TwBucket::erase;
     using TwBucket::size;
     using TwBucket::path;
     using TwBucket::push_back;
     using TwBucket::feasable;
     using TwBucket::twvTot;
     using TwBucket::cvTot;
      ...
      */
     bool e_insert( const Trashnode &node, POS at, double maxcapacity ) {
         assert (at <= size());
      ...
      */
     bool e_push_back(const Trashnode &node, double maxcapacity) {
...
#ifdef USE
     bool operator ==( const Twpath<Trashnode> &other ) {
     ...
     Twpath &operator =( const TwBucket &other ) {
         TwBucket::operator = ( other );
         return *this;
     }
     ///@}
     ...
     // dosnt move dumps
     bool e__insert( const Trashnode &n, UID at, double maxcapacity ) {
#ifdef TESTED
     ...
     bool e__adjustDumpsToMaxCapacity( int currentPos, const Trashnode &dumpS,
             double maxcapacity ) {
      ...
      */
     bool e_swap(POS i, double maxcap, Twpath<Trashnode> &rhs, POS j,
             double rhs_maxcap) {
